package com.zenyte.game.content.flowerpoker;

import com.zenyte.game.world.entity.player.Player;
import com.zenyte.game.world.region.PolygonRegionArea;
import com.zenyte.game.world.region.RSPolygon;
import com.zenyte.game.world.region.area.plugins.LayableObjectPlugin;
import org.jetbrains.annotations.NotNull;

public final class FlowerPokerEdgevilleArea extends PolygonRegionArea implements LayableObjectPlugin {

    private static final RSPolygon POLYGON = new RSPolygon(
            new int[][]{
                    {3076, 3460}, // zuidwest
                    {3098, 3460}, // zuidoost
                    {3098, 3478}, // noordoost
                    {3076, 3478}  // noordwest
            }
    );

    @Override
    public RSPolygon[] polygons() {
        return new RSPolygon[]{POLYGON};
    }

    @Override
    public void enter(Player player) {
        player.setPlayerGambleable(true);
    }

    @Override
    public void leave(Player player, boolean logout) {
        player.setPlayerGambleable(false);
    }

    @Override
    public String name() {
        return "Flower Poker Edgeville Area";
    }

    @Override
    public boolean canLay(@NotNull Player player, @NotNull LayableObjectType type) {
        player.sendMessage("You can not do this here.");
        return false;
    }
}
